...
static Layer *Layer_create(
    ...
    int nbiases, int nweights, size_t *used_memory)
{
...
*/
Layer *Layer_create_input(int depth, int width, int height, size_t *used_memory)
{
    return Layer_create(
        NULL, LAYER_INPUT, depth, width, height, 0, 0, used_memory);
}

/* Layer_create_full(lprev, nnodes, std)
   Creates a fully-connected Layer.
*/
Layer *Layer_create_full(Layer *lprev, int nnodes, double std, size_t *used_memory)
{
    assert(lprev != NULL);
    Layer *self = Layer_create(
        lprev, LAYER_FULL, nnodes, 1, 1,
        nnodes, nnodes * lprev->nnodes, used_memory);
    assert(self != NULL);
...
Layer *Layer_create_conv(
    ...
    int kernsize, int padding, int stride, double std, size_t *used_memory)
{
    assert(lprev != NULL);
    assert((kernsize % 2) == 1);
    ...
    Layer *self = Layer_create(
        ...
        depth, depth * lprev->depth * kernsize * kernsize, used_memory);
    assert(self != NULL);
